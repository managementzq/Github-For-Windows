...
namespace Tianyue.Utility.Helper
{
    ...
    public class AssemblyCacheHelper
    {
        ...
        public static Assembly GetAssembly(string dll)
        {
            ...
            Assembly asm = null;
            if (CommonCacheManager.AssemblyCache.TryGetValue(assemblyName, out asm))
            {
                return asm;
            }
            else
            {
                try
                {
                    ...
                    if (asm != null)
                    {
                        CommonCacheManager.AssemblyCache.Add(assemblyName, asm);
                        PreJitControls(asm);
                    }
                }
                catch (Exception)
        ...
        public static Type GetType(Assembly asm, string typename)
        {
            ...
            Type t = null;
            if (CommonCacheManager.TypeCache.TryGetValue(typename, out t)) //比ContainKey性能好很多
            {
                return t;
            }
            else
            {
                ...
                CommonCacheManager.TypeCache.Add(typename, t);
            }
            return t;
        }
            ...
            NetworkCredential cdt = null)
        {
            ...
            if (CommonCacheManager.AssemblyCache.ContainsKey(upperURL))
            {
                assembly = (Assembly)CommonCacheManager.AssemblyCache[upperURL];
            }
            else
            {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    using (Stream stream = wc.OpenRead(url + "?wsdl"))
                    {
                        ...
                        CommonCacheManager.AssemblyCache[upperURL] = assembly;
                    }
                }
            }
            return assembly;
        ...
        {
            CommonCacheManager.AssemblyCache.Clear();
            CommonCacheManager.TypeCache.Clear();
        }
    }
}
